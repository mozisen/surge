...
if (typeof $request !== "undefined") {
  ...
  const version = header($request.headers, "refract-version");
  if (cookie) write(cookie, KEY.cookie);
  if (ua) write(ua, KEY.ua);
  if (version) write(version, KEY.version);
  notify(cookie ? "Cookie 获取成功" : "Cookie 获取失败", cookie ? "已保存在 Surge 本机，可等待定时签到。" : "本次请求未携带 Cookie，请确认已登录。");
  $done({});
} else {
...
async function run() {
  ...
  const url = HOME + "/api/attendance?random=true";
  ...
  if (status >= 200 && status < 300 && (payload.success || /鸡腿|已完成签到|签到/.test(message))) notify("签到成功", message);
