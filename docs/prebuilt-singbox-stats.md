...
包源为本项目 `singbox-stats-binaries` 专用分支，每个版本含四种平台组合的归档及 manifest。包由 GitHub Actions 从官方固定版本源码构建，CGO 禁用；运行 VLESS、AnyTLS、Trojan 的真实回环计数/reset 测试，四组全部通过才发布。已有平台包目录不被重复构建覆盖。

下载时校验版本/架构、归档和二进制 SHA-256，再检查核心版本、统计标签与现有配置。保留现有备份、服务重启、API 健康检查及失败恢复流程。仍需归档、解压核心和旧核心备份空间，但不再需要完整编译环境的磁盘空间。

## 构建新版本

GitHub Actions → Build precompiled Sing-box statistics cores → Run workflow，选择 `codex/singbox-prebuilt-stats` 分支，填写官方稳定版本（例如 1.14.0）。构建成功后服务器才可下载该版本。此流程不自动跟踪上游、不创建 Release；未来版本仍需实际编译和测试，不能预先保证支持。

工作流及 CI 构建脚本可独立部署到主分支，正式安装脚本仍保持 3.7.0；新下载逻辑在 `codex/singbox-prebuilt-stats` 预览分支测试。

## 验证

十套离线回归测试及 Bash 语法/diff 检查通过，包括四平台下载、libc 不匹配、缺包、损坏包、未知架构、1.14 补丁版本数字排序、预发布过滤及网络失败。

1.14.0 四组真实计数测试和发布已通过：[CI 34739273348](https://github.com/mozisen/surge/actions/runs/34739273348)。四个公开包实际下载及 SHA-256 校验通过。Linux 服务替换/自动回滚流程仍需用户测试机验证。
