...
: "${CORE_LIBC:?}"
[[ "$CORE_LIBC" == glibc || "$CORE_LIBC" == musl ]]
...
out="dist/v$CORE_VERSION/linux-$CORE_ARCH-$CORE_LIBC"
mkdir -p "$out"
tar -czf "$out/sing-box.tar.gz" -C "$work/bin" sing-box
jq -n --arg version "$CORE_VERSION" --arg arch "$CORE_ARCH" --arg libc "$CORE_LIBC" --arg tags "$tags" \
  ...
  '{version:$version,arch:$arch,libc:$libc,os:"linux",profile:"stats-v1",tags:$tags,source:$source,workflow_commit:$workflow_commit,binary_sha256:$binary_sha256,archive_sha256:$archive_sha256}' > "$out/manifest.json"
