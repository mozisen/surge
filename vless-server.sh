...
# AnyTLS TLS 证书交互配置
# 设置全局变量：ANYTLS_CERT_MODE / ANYTLS_CERT_SNI
configure_anytls_certificate() {
    ANYTLS_CERT_MODE=""
    ANYTLS_CERT_SNI=""
    mkdir -p "$CFG/certs"
    ...
    while true; do
        echo ""
        _line
        echo -e "  ${W}AnyTLS TLS 证书配置${NC}"
        _line
        echo -e "  ${G}1)${NC} 使用已有证书 (${CFG}/certs/server.crt)"
        echo -e "  ${G}2)${NC} 使用域名申请 Let's Encrypt 证书"
        echo -e "  ${G}3)${NC} 使用自签证书"
        echo ""
        read -rp "  请选择 [1-3，默认 3.自签证书]: " cert_choice
        cert_choice="${cert_choice:-3}"

        case "$cert_choice" in
            1)
                if [[ ! -s "$CFG/certs/server.crt" || ! -s "$CFG/certs/server.key" ]]; then
                    _warn "未找到已有证书或私钥，请重新选择"
                    continue
                fi
                if ! openssl x509 -in "$CFG/certs/server.crt" -noout >/dev/null 2>&1; then
                    _warn "已有证书格式无效，请重新选择"
                    continue
                fi

                local detected_sni=""
                detected_sni=$(openssl x509 -in "$CFG/certs/server.crt" -noout -ext subjectAltName 2>/dev/null \
                    | grep -oE 'DNS:[^, ]+' | head -n1 | cut -d: -f2-)
                [[ -z "$detected_sni" ]] && detected_sni=$(openssl x509 -in "$CFG/certs/server.crt" -noout -subject 2>/dev/null \
                    | sed -n 's/.*CN *= *\([^,]*\).*/\1/p')
                ...
                ANYTLS_CERT_SNI="$input_sni"
                _ok "将使用已有证书，SNI: $ANYTLS_CERT_SNI"
                return 0
                ;;
            2)
                echo -e "  ${Y}提示：域名必须已解析到本机公网 IP，且 80/443 端口满足证书签发要求${NC}"
                local domain=""
                while true; do
                ...
                NGINX_PORT="18443"
                echo "$NGINX_PORT" > "$CFG/.nginx_port_tmp" 2>/dev/null
                if get_acme_cert "$domain" "anytls"; then
                    echo "$domain" > "$CFG/cert_domain"
                    rm -f "$CFG/.nginx_port_tmp"
                    ANYTLS_CERT_MODE="acme"
                    ANYTLS_CERT_SNI="$domain"
                    _ok "真实证书申请成功，SNI: $domain"
                    return 0
                fi
                rm -f "$CFG/.nginx_port_tmp"
                _warn "证书申请失败，请重新选择证书方式"
                ;;
            3)
                ...
                rm -f "$CFG/certs/server.crt" "$CFG/certs/server.key"
                _info "生成带 SAN 的 AnyTLS 自签证书..."
                if ! openssl req -x509 -nodes -newkey ec:<(openssl ecparam -name prime256v1) \
                    -keyout "$CFG/certs/server.key" -out "$CFG/certs/server.crt" \
                    -subj "/CN=$self_sni" -days 36500 \
                    -addext "subjectAltName=DNS:$self_sni" \
                    -addext "basicConstraints=critical,CA:FALSE" \
                ...
                chmod 600 "$CFG/certs/server.key"
                echo "$self_sni" > "$CFG/cert_domain"
                ANYTLS_CERT_MODE="self"
                ANYTLS_CERT_SNI="$self_sni"
                _ok "自签证书生成完成，SNI: $self_sni"
                return 0
                ;;
            *)
                _warn "无效选项，请输入 1、2 或 3"
                ;;
        esac
    done
}
...
gen_anytls_server_config() {
    local password="$1" port="$2" sni="${3:-bing.com}" cert_mode="${4:-self}"
    mkdir -p "$CFG"

    # 证书必须在安装交互步骤中准备完成，不再静默生成或覆盖
    if [[ ! -s "$CFG/certs/server.crt" || ! -s "$CFG/certs/server.key" ]]; then
        _err "AnyTLS TLS 证书或私钥不存在"
        return 1
    fi
...
# Snell v6 服务端配置
# v6 已移除 obfs/ipv6 字段，新增 DNS、dns-ip-preference、mode 与 TCP Fast Open
gen_snell_v6_server_config() {
    local psk="$1" port="$2" version="${3:-6}"
    local tfo="${4:-true}"
    local dns_servers="${5:-1.1.1.1, 8.8.8.8, 2001:4860:4860::8888}"
    local dns_pref="${6:-default}"
    local mode="${7:-default}"
    mkdir -p "$CFG"
...
EOF
    ...
    register_protocol "snell-v6" "$(build_config psk "$psk" port "$port" version "$version" tfo "$tfo" dns "$dns_servers" dns_ip_preference "$dns_pref" mode "$mode")"
    ...
    case "$protocol" in
        ...
        snell|snell-v5|snell-v6)
            ...
            if [[ "$protocol" == "snell" ]]; then
                ...
                # Snell v6 PSK：仅允许字母和数字，长度 16-255
                while true; do
                    echo ""
                    echo -e "  ${G}请输入 Snell Server 密钥 [0-9][a-z][A-Z]${NC}"
                    echo -e "  ${Y}[提示] 当前目标协议为 Snell v6，密钥长度要求在 16-255 位之间${NC}"
                    read -rp "  (默认：随机生成): " psk
                    [[ -z "$psk" ]] && psk=$(gen_password 32)
                    if [[ ${#psk} -ge 16 && ${#psk} -le 255 && "$psk" =~ ^[0-9A-Za-z]+$ ]]; then
                        break
                    fi
                    _err "密钥格式错误：只能包含数字和英文字母，长度必须为 16-255 位"
                done
            fi
            ...
            if [[ -n "$stls_protocol" && "$enable_stls_pre" =~ ^[yY]$ ]]; then
                ...
                local snell_v6_tfo=""
                local snell_v6_dns=""
                local snell_v6_dns_pref=""
                local snell_v6_mode=""

                if [[ "$version" == "6" ]]; then
                    # 1. TCP Fast Open：必须明确选择
                    while true; do
                        echo ""
                        echo -e "  ${C}是否开启 TCP Fast Open?${NC}"
                        _line
                        echo -e "  ${G}1. 开启${NC}"
                        echo -e "  ${G}2. 关闭${NC}"
                        _line
                        read -rp "  请选择 [1-2] (默认: 1.开启): " tfo_choice
                        tfo_choice="${tfo_choice:-1}"
                        case "$tfo_choice" in
                    ...
                    # 2. DNS：预设方案或自定义，必须明确选择
                    while true; do
                        echo ""
                        echo -e "  ${C}请选择 Snell Server DNS${NC}"
                        _line
                        echo -e "  ${G}1. Cloudflare${NC}  1.1.1.1, 1.0.0.1"
                        echo -e "  ${G}2. Google${NC}      8.8.8.8, 8.8.4.4"
                        echo -e "  ${G}3. Quad9${NC}       9.9.9.9, 149.112.112.112"
                        echo -e "  ${G}4. 阿里 DNS${NC}    223.5.5.5, 223.6.6.6"
                        echo -e "  ${G}5. 腾讯 DNSPod${NC} 119.29.29.29, 182.254.116.116"
                        echo -e "  ${G}6. IPv4 + IPv6${NC} 1.1.1.1, 8.8.8.8, 2606:4700:4700::1111, 2001:4860:4860::8888"
                        echo -e "  ${G}7. 自定义 DNS${NC}"
                        _line
                        read -rp "  请选择 [1-7] (默认: 6.IPv4 + IPv6): " dns_choice
                        dns_choice="${dns_choice:-6}"
                        case "$dns_choice" in
                            1) snell_v6_dns="1.1.1.1, 1.0.0.1"; break ;;
                            2) snell_v6_dns="8.8.8.8, 8.8.4.4"; break ;;
                            3) snell_v6_dns="9.9.9.9, 149.112.112.112"; break ;;
                            4) snell_v6_dns="223.5.5.5, 223.6.6.6"; break ;;
                            5) snell_v6_dns="119.29.29.29, 182.254.116.116"; break ;;
                            6) snell_v6_dns="1.1.1.1, 8.8.8.8, 2606:4700:4700::1111, 2001:4860:4860::8888"; break ;;
                            7)
                                while true; do
                                    echo -e "  ${Y}[提示] 多个 DNS 请用英文逗号分隔。${NC}"
                                    read -rp "  请输入自定义 DNS: " dns_input
                                    # 接受 IPv4/IPv6/域名形式，但拒绝空值及明显危险字符
                                    if [[ -n "$dns_input" && "$dns_input" =~ ^[0-9A-Za-z:.,_-]+([[:space:]]*,?[[:space:]]*[0-9A-Za-z:._-]+)*$ ]]; then
                                        snell_v6_dns="$dns_input"
                                        break 2
                                    fi
                                    _err "DNS 格式无效，请重新输入"
                                done
                    ...
                    while true; do
                        echo ""
                        echo -e "  ${C}配置 DNS IP 偏好 (Snell v6 专属)${NC}"
                        _line
                        echo -e "  ${G}1. default${NC}"
                        echo -e "  ${G}2. prefer-ipv4${NC}"
                        echo -e "  ${G}3. prefer-ipv6${NC}"
                        echo -e "  ${G}4. ipv4-only${NC}"
                        echo -e "  ${G}5. ipv6-only${NC}"
                        _line
                        read -rp "  请选择 [1-5] (默认: 1.default): " dns_pref_choice
                        dns_pref_choice="${dns_pref_choice:-1}"
                        case "$dns_pref_choice" in
                            1) snell_v6_dns_pref="default"; break ;;
                            2) snell_v6_dns_pref="prefer-ipv4"; break ;;
                            3) snell_v6_dns_pref="prefer-ipv6"; break ;;
                            4) snell_v6_dns_pref="ipv4-only"; break ;;
                            5) snell_v6_dns_pref="ipv6-only"; break ;;
                            *) _err "无效选择，请输入 1-5" ;;
                        esac
                    done
                    ...
                    while true; do
                        ...
                        _line
                        echo -e "  ${G}1. default${NC}"
                        echo -e "  ${G}2. unshaped${NC}"
                        echo -e "  ${G}3. unsafe-raw${NC}"
                        _line
                        read -rp "  请选择 [1-3] (默认: 1.default): " mode_choice
                        mode_choice="${mode_choice:-1}"
                        case "$mode_choice" in
                            1) snell_v6_mode="default"; break ;;
                            2) snell_v6_mode="unshaped"; break ;;
                            3) snell_v6_mode="unsafe-raw"; break ;;
                            *) _err "无效选择，请输入 1-3" ;;
                        esac
                    done
                fi
                ...
                if [[ "$version" == "4" ]]; then
                    ...
                    gen_snell_v6_server_config "$psk" "$port" "$version" "$snell_v6_tfo" "$snell_v6_dns" "$snell_v6_dns_pref" "$snell_v6_mode"
                fi
            fi
            ;;
        tuic)
        ...
        anytls)
            ...
            # AnyTLS 必须明确选择 TLS 证书方式
            configure_anytls_certificate || return 1
            local final_sni="$ANYTLS_CERT_SNI"
            local cert_mode="$ANYTLS_CERT_MODE"
            ...
            echo -e "  端口: ${G}$port${NC}"
            echo -e "  密码: ${G}$password${NC}"
            echo -e "  证书方式: ${G}$cert_mode${NC}"
            echo -e "  SNI: ${G}$final_sni${NC}"
            echo -e "  证书: ${G}$CFG/certs/server.crt${NC}"
            echo -e "  私钥: ${G}$CFG/certs/server.key${NC}"
            _line
            echo ""
            read -rp "  确认安装? [Y/n]: " confirm
            [[ "$confirm" =~ ^[nN]$ ]] && return
            ...
            _info "生成配置..."
            gen_anytls_server_config "$password" "$port" "$final_sni" "$cert_mode"
            ;;
        naive)
