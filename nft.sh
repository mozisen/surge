...
install_shortcut() {
    ...
    mkdir -p "${INSTALL_DIR}" /usr/local/bin 2>/dev/null || {
        warn "无法创建快捷命令目录，已跳过快捷命令安装。"
        return 0
    }

    # 脚本不在固定目录时，复制一份到固定位置；以后删除下载文件也不受影响。
    if [[ "${current_script}" != "${INSTALL_SCRIPT}" ]]; then
        if ! install -m 755 "${current_script}" "${INSTALL_SCRIPT}" 2>/dev/null; then
            warn "无法安装脚本到 ${INSTALL_SCRIPT}，已跳过快捷命令安装。"
            return 0
        fi
    else
    ...
    if [[ ! -L "${SHORTCUT_PATH}" ]] || [[ "$(readlink -f "${SHORTCUT_PATH}" 2>/dev/null)" != "${INSTALL_SCRIPT}" ]]; then
        if ln -sfn "${INSTALL_SCRIPT}" "${SHORTCUT_PATH}" 2>/dev/null; then
            info "快捷命令已创建：以后输入 ${SHORTCUT_NAME} 即可打开管理工具。"
            log_action "创建快捷命令 ${SHORTCUT_PATH} -> ${INSTALL_SCRIPT}"
        else
            warn "快捷命令创建失败，请手动执行：ln -sfn ${INSTALL_SCRIPT} ${SHORTCUT_PATH}"
        fi
    fi
}
